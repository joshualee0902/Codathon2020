...
def validateSeq(dna_seq):
    tmpseq = dna_seq.upper()  # needs to be uppercase because of ASCII values
    for nuc in tmpseq:
...
def countNucFrequency(seq):
    ...
    for nuc in seq:
        tmpFreqDict[nuc] += 1
    return tmpFreqDict
    # return dict(collections.Counter(seq))   #alternative method using collections library
...
def protenis_from_rf(aa_seq):
    """Compute all possible proteines in an aminoacid sequence and return a list of possible proteins"""
    current_prot = []
    proteins = []
    for aa in aa_seq:
        if aa == "_":
